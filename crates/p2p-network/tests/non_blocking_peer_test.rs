...
async fn test_non_blocking_peer_communication_setup() -> Result<()> {
    ...
    info!(
        "Peer 1 stats: connections={}, total={}",
        stats1.active_connections, stats1.total_connections
    );
    info!(
        "Peer 2 stats: connections={}, total={}",
    ...
    let data_request_1 = peer1
        .request_blockchain_data("transaction".to_string(), tx2_to_1.hash.clone())
        .await;

    let data_request_2 = peer2
        .request_blockchain_data("transaction".to_string(), tx1_to_2.hash.clone())
        .await;

    info!(
        "Data request results - Peer1: {:?}, Peer2: {:?}",
        data_request_1.is_ok(),
        data_request_2.is_ok()
    );
    ...
    info!(
        "Peer 1 - Discovered: {}, Connected: {}",
        discovered1.len(),
        connected1.len()
    );
    info!(
        "Peer 2 - Discovered: {}, Connected: {}",
        discovered2.len(),
        connected2.len()
    );
...
async fn test_multi_peer_network_simulation() -> Result<()> {
    ...
    assert_eq!(
        successful_broadcasts, 5,
        "All peers should broadcast successfully"
    );
    info!(
        "All {} peers can broadcast transactions",
    ...
    for (i, peer) in peers.iter().enumerate() {
        if peer
            .adaptive_broadcast_transaction(&global_tx)
            .await
            .is_ok()
        {
        ...
        info!(
            "Peer {} adaptive stats - Discovered: {}, Connected: {}, DHT: {}",
            i,
    ...
    assert_eq!(
        adaptive_broadcasts, 5,
        "All peers should support adaptive broadcast"
    );

    // Test cross-peer data requests
    for (i, peer) in peers.iter().enumerate() {
        let target_peer = (i + 2) % peers.len();
        let request_result = peer
    ...
    for (i, peer) in peers.iter().enumerate() {
        ...
        info!(
            "Peer {} final stats - Messages: {}, Discovered: {}, Connected: {}",
            i,
            stats.messages_sent,
            discovered.len(),
            connected.len()
...
async fn test_peer_connection_resilience() -> Result<()> {
    ...
    for (i, peer) in peers.iter().enumerate() {
        let tx =
            create_test_transaction(300 + i as u64, &format!("resilient_{}", i), "network", 1000);
        let result = peer.broadcast_transaction(&tx).await;
        assert!(result.is_ok(), "Peer {} should be functional", i);

        let stats = peer.get_network_stats();
        info!(
            "Peer {} initial stats: messages={}, connections={}",
    ...
    for (i, peer) in peers.iter().enumerate() {
        let tx =
            create_test_transaction(400 + i as u64, &format!("surviving_{}", i), "network", 1500);
        let result = peer.broadcast_transaction(&tx).await;
        ...
        let adaptive_stats = peer.get_adaptive_network_stats().await;
        info!(
            "Surviving peer {} - Discovered: {}, DHT: {}, Efficiency: {:.2}",
            i,
            discovered.len(),
            adaptive_stats.dht_nodes_count,
    ...
    for (i, peer) in peers.iter().enumerate() {
        let tx =
            create_test_transaction(500 + i as u64, &format!("recovered_{}", i), "network", 2000);
        if peer.broadcast_transaction(&tx).await.is_ok() {
            recovered_peers += 1;
        }
        ...
        let adaptive_stats = peer.get_adaptive_network_stats().await;
        info!(
            "Recovery peer {} - Messages: {}, DHT nodes: {}, Discovered: {}",
            i,
            stats.messages_sent,
            adaptive_stats.dht_nodes_count,
            adaptive_stats.discovered_peers_count
    ...
    assert_eq!(
        recovered_peers,
        peers.len(),
        "All remaining peers should work after recovery"
    );
    info!(
        "Network recovery successful: {}/{} peers functional",
        recovered_peers,
        peers.len()
    );
...
async fn test_peer_broadcast_patterns() -> Result<()> {
    ...
    for (i, result) in concurrent_results.iter().enumerate() {
        match result {
            Ok(broadcasts) => {
                ...
                info!(
                    "Peer {} concurrent broadcasts: {}/3 successful",
                    i, successful
                );
                assert_eq!(
                    successful, 3,
                    "All concurrent broadcasts should succeed for peer {}",
                    i
                );
            }
            Err(e) => {
    ...
    for (i, peer) in peers.iter().enumerate() {
        ...
        assert!(
            result.is_ok(),
            "Peer {} adaptive broadcast should succeed",
            i
        );

        let adaptive_stats = peer.get_adaptive_network_stats().await;
        info!(
            "Peer {} adaptive broadcast stats - Efficiency: {:.2}, DHT: {}",
    ...
    for (i, peer) in peers.iter().enumerate() {
        let stats = peer.get_network_stats();
        info!(
            "Peer {} final broadcast stats - Total messages: {}, Connections: {}",
            i, stats.messages_sent, stats.total_connections
        );
    }
