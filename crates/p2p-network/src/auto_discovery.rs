...
pub enum SimpleDiscoveryMessage {
    ...
    PeerResponse { peers: Vec<SimplePeerInfo> },
}
            ...
            tokio::spawn(async move {
                ...
                loop {
                    match socket.recv_from(&mut buffer).await {
                        Ok((len, from)) => {
                            if let Ok(message) =
                                bincode::deserialize::<SimpleDiscoveryMessage>(&buffer[..len])
                            {
                                match message {
                                    SimpleDiscoveryMessage::Announce {
                                        node_id: peer_id,
                                        address,
                                        port: _,
                                    } => {
                                    ...
                                    SimpleDiscoveryMessage::PeerRequest { node_id: _ } => {
                                        ...
                                        let peer_list: Vec<SimplePeerInfo> =
