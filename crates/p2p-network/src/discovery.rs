...
impl PeerDiscovery {
    ...
    pub async fn new(node_id: String, listen_addr: SocketAddr) -> Result<Self> {
        ...
        Ok(Self {
            ...
            connection_pool: Arc::new(Mutex::new(ConnectionPool::new(50, Duration::from_secs(300)))),
            multicast_addr,
        })
    }
        ...
        let listen_task = tokio::spawn(async move {
            ...
            loop {
                match socket.recv_from(&mut buffer).await {
                    Ok((len, from)) => {
                        if let Ok(message) = bincode::deserialize::<DiscoveryMessage>(&buffer[..len]) {
                            Self::handle_discovery_message(&node_id, &known_peers, message, from, &socket).await;
                        }
                    }
                    Err(e) => {
    ...
    async fn handle_discovery_message(
        ...
        match message {
            DiscoveryMessage::Announce { node_id: peer_id, address, capabilities } => {
                if peer_id != node_id {
            ...
            DiscoveryMessage::Query { target_id: _, max_peers } => {
            ...
            DiscoveryMessage::Ping { node_id: _peer_id, timestamp } => {
    ...
    pub async fn add_peer(&self, peer: PeerInfo) {
        self.known_peers.write().await.insert(peer.node_id.clone(), peer);
    }
...
impl ConnectionPool {
    ...
    pub fn add_connection(&mut self, peer_id: String, address: SocketAddr) -> Result<()> {
        if self.active_connections.len() >= self.max_connections {
            ...
            if let Some((oldest_id, _)) = self.active_connections
                .iter()
    ...
    pub fn cleanup_stale_connections(&mut self) {
        ...
        self.active_connections.retain(|_, connection| {
            now.duration_since(connection.last_activity).unwrap_or(Duration::MAX) < self.connection_timeout
        });
    }
    ...
    pub fn get_best_connections(&self, limit: usize) -> Vec<&ConnectionInfo> {
        ...
        connections.sort_by(|a, b| b.connection_quality.partial_cmp(&a.connection_quality).unwrap());
...
impl DHT {
    ...
    pub async fn is_bucket_full(&self, target_distance: &[u8; 20]) -> bool {
        let table = self.routing_table.read().await;
        let nodes_at_distance: Vec<_> = table.values()
            .filter(|node| {
...
mod tests {
    ...
    async fn test_dht_operations() {
        ...
        dht.add_node("node1".to_string(), "127.0.0.1:8001".parse().unwrap()).await;
        dht.add_node("node2".to_string(), "127.0.0.1:8002".parse().unwrap()).await;
    ...
    fn test_connection_pool() {
        ...
        pool.add_connection("peer1".to_string(), "127.0.0.1:8001".parse().unwrap()).unwrap();
        pool.add_connection("peer2".to_string(), "127.0.0.1:8002".parse().unwrap()).unwrap();
        
        assert_eq!(pool.get_active_connections().len(), 2);
        
        // Adding third connection should remove oldest
        pool.add_connection("peer3".to_string(), "127.0.0.1:8003".parse().unwrap()).unwrap();
        assert_eq!(pool.get_active_connections().len(), 2);
    }
}