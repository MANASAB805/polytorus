...
impl ScriptStateManager {
    ...
    pub fn deploy_script(
        ...
        // Store script
        self.scripts.lock().unwrap().insert(script_hash.clone(), metadata);
        ...
        self.states.lock().unwrap().insert(script_hash.clone(), script_state);
    ...
    pub fn update_state(
        ...
        let new_size = script_state.total_size + value.len() - 
            script_state.state.get(&key).map(|e| e.value.len()).unwrap_or(0);
...
mod tests {
    ...
    fn test_script_deployment() {
        ...
        let script_hash = manager.deploy_script(
            "alice".to_string(),
    ...
    fn test_export_import() {
        ...
        let script_hash = manager1.deploy_script(
            "alice".to_string(),
            ScriptType::BuiltIn(BuiltInScript::PayToPublicKey),
            vec![1, 2, 3],
            Some("Test".to_string()),
