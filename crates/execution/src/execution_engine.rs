...
use traits::{
    Event, Hash, Result, ScriptContext, Utxo, UtxoExecutionBatch, UtxoExecutionLayer,
    UtxoExecutionResult, UtxoId, UtxoSet, UtxoTransaction, UtxoTransactionReceipt,
};
...
impl PolyTorusUtxoExecutionLayer {
    ...
    pub fn new(config: UtxoExecutionConfig) -> Result<Self> {
        ...
        linker.func_wrap(
            "env",
            "get_utxo_value",
            |caller: wasmtime::Caller<'_, ScriptExecutionStore>, utxo_index: u32| -> u64 {
        ...
        linker.func_wrap(
            "env",
            "get_current_slot",
            |caller: wasmtime::Caller<'_, ScriptExecutionStore>| -> u64 {
                let store_data = caller.data();
                if let Some(ref ctx) = store_data.script_context {
        ...
        linker.func_wrap(
            "env",
            "validate_signature",
            |mut caller: wasmtime::Caller<'_, ScriptExecutionStore>,
             _pub_key_ptr: u32,
             signature_ptr: u32,
             message_ptr: u32|
             -> i32 {
                ...
                let signature_data =
                    &data[signature_start..signature_start + 64.min(data.len() - signature_start)];
    ...
    fn process_utxo_transaction(&mut self, tx: &UtxoTransaction) -> Result<UtxoTransactionReceipt> {
            ...
            match self.execute_script(&utxo.script, &input.redeemer, &script_context) {
                Ok(valid) => {
                    if !valid {
                        success = false;
                        script_logs
                            .push(format!("Script validation failed for input {input_index}"));
                    }
                ...
                Err(e) => {
                    success = false;
                    script_logs.push(format!(
                        "Script execution error for input {input_index}: {e}"
                    ));
                }
            }
        }
        ...
        if success {
            ...
            for input in &tx.inputs {
                ...
                utxo_set.total_value -= consumed_utxos
                    .iter()
    ...
    fn update_execution_context(&self, receipt: &UtxoTransactionReceipt) {
        if let Ok(mut context_guard) = self.execution_context.lock() {
            if let Some(ref mut context) = *context_guard {
                ...
                context
                    .consumed_utxos
                    .extend(receipt.consumed_utxos.clone());
    ...
    pub fn initialize_genesis_utxo_set(
        &mut self,
        genesis_utxos: Vec<(UtxoId, Utxo)>,
    ) -> Result<Hash> {
        let mut utxo_set = self.utxo_set.lock().unwrap();
        ...
        if !utxo_set.utxos.is_empty() {
            return Err(anyhow::anyhow!(
                "Cannot initialize genesis UTXO set: UTXO set is not empty"
            ));
        }
        ...
        for (utxo_id, utxo) in genesis_utxos {
            ...
            if utxo.id != utxo_id {
                return Err(anyhow::anyhow!(
                    "Genesis UTXO ID mismatch: expected {:?}, got {:?}",
                    utxo_id,
                    utxo.id
                ));
            }
        ...
        log::info!(
            "Initialized genesis UTXO set with {} UTXOs, total value: {}",
            utxo_set.utxos.len(),
            total_value
        );
    ...
    pub fn create_coinbase_utxo(
        ...
        log::info!(
            "Created coinbase UTXO {} with value {}",
            utxo_id.tx_hash,
            reward
        );
...
impl UtxoExecutionLayer for PolyTorusUtxoExecutionLayer {
    async fn execute_utxo_transaction(
        ...
        match self.process_utxo_transaction(tx) {
            Ok(receipt) => {
                log::info!(
                    "UTXO transaction execution completed successfully: {}",
                    tx.hash
                );
                Ok(receipt)
            }
    ...
    async fn execute_utxo_batch(
        ...
        let execution_result = UtxoExecutionResult {
            ...
            consumed_utxos: all_receipts
                .iter()
                .flat_map(|r| r.consumed_utxos.clone())
                .collect(),
            created_utxos: {
                let utxo_set = self.utxo_set.lock().unwrap();
                all_receipts
    ...
    async fn get_utxos_by_script(&self, script_hash: &Hash) -> Result<Vec<Utxo>> {
        ...
        let matching_utxos: Vec<Utxo> = utxo_set
            .utxos
            .values()
            .filter(|utxo| {
                hasher.update(&utxo.script);
    ...
    async fn commit_utxo_execution(&mut self) -> Result<Hash> {
        ...
        if let Some(ctx) = context {
            ...
            log::info!(
                "UTXO set hash changed from {} to {}",
                ctx.initial_utxo_set_hash,
                current_hash
            );
